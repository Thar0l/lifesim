...
Settings::Settings(std::string filename)
{
	...
	{
		std::cout << "Settings loaded from settings.ini\n";
	}
		 ...
		 con_food_spend = reader.GetInteger("Unit", "con_food_spend", 4);
		 con_food_heal = reader.GetInteger("Unit", "con_food_heal", 5);
		 con_food_energy = reader.GetInteger("Unit", "con_food_energy", 5);
		 con_food_split = reader.GetInteger("Unit", "con_food_split", 12000);
		 ...
		 start_units = reader.GetInteger("World", "start_units", 60);
		 max_units = reader.GetInteger("World", "max_units", 1200);
